...
다른 브랜치의 마지막 커밋(가장 최근 커밋)으로 깃 디렉토리를 변경한다. 이 때 워킹 트리와 스테이징 에어리어가 깨끗하다면 둘을 깃 디렉토리와 같은 상태로 만든다.
...
깃 디렉토리에서 달라진 상태(unstaged, untracked file, modified 같은)의 파일이 있다면 경우에 따라 브랜치 이동에 실패하거나, 브랜치를 이동하되 달라진 상태의 파일은 유지한다. 실패하는 경우는 보통 현재 브랜치와 이동하려는 브랜치 양쪽에 존재하는 파일이 변경되었지만 커밋을 하지 않았을 때다.
