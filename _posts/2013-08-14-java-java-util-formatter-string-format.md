...
부동소수점 숫자를 표시할 때 소수점 아래 자릿수를 얼마나 표시할 지를 결정하는 값이다. 정수일 땐 무시된다.
...
부동소수점 숫자를 지수 표기법(Exponential notation, scientific notation)으로 치환한다.
...
부동소수점 숫자를 일반 형식으로 치환한다. 일정 자릿수 이하는 반올림하며(기준은 몲) 소수점 100만이 넘는 숫자는 지수 표기법을 쓴다.
...
부동소수점 숫자를 16진수 부동소수점 표기법으로 치환한다.
