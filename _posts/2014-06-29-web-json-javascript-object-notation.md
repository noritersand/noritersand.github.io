...
JSON은 기본적으로 이름과 값을 쌍으로 갖는 형태를 취한다.

```json
{ "a": 123 };
```

값은 문자열, 숫자, 불리언, null, 배열과 또다른 JSON 객체만이 허용되며, 이름과 값의 한 쌍을 프로퍼티라고 할 때, 하나 이상의 프로퍼티는 쉼표로 구분한다.


**자바스크립트의 객체 리터럴 표기법(Object literal notation)과는 문법적인 차이가 있다.** 둘의 가장 큰 차이점은 다음과 같다:
