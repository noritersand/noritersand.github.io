...
데이터를 표현하는 방법 중 하나. 뜻을 직역하면 JavaScript 객체 표기법이다. 경량이며 읽고 쓰기 편리하다는 특징이 있다. 이름과는 달리 특정 프로그래밍 언어에 의존하지 않는다. 그래서 XML이나 CSV와 같은 범용 데이터 교환 포맷으로 사용된다.

**JavaScript의 객체 리터럴 표기법(Object literal notation)과는 문법 차이가 있다.** 둘의 가장 큰 차이점은 다음과 같다:

- JSON은 함수를 정의할 수 없다.
- JSON에서 이름과 값(문자열에만 해당)을 감싸는 큰따옴표는 생략하거나 작은따옴표로 대체할 수 없다.
...
최상위 객체는 배열로 대체할 수 있다. JSON array의 표기법은 JavaScript와 같다. 대괄호`[]`로 감싸진 쉼표로 구분된 값의 나열이다.
...
JavaScript는 JSON을 다루기 위한 네이티브 메서드를 제공한다. 이 중 `JSON.parse()`는 JSON 문자열을 JavaScript 객체로 변환하여 반환하는데, 이 때 전달된 인자가 JSON 규격에 맞는지 엄격히 체크한다.
...
- `jsObject`: 문자열로 변환할 객체

`JSON.stringify()`는 JavaScript 객체를 JSON 문자열로 변환하는 메서드다.
