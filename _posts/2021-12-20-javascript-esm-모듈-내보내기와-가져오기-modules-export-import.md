...
참고로 ESM이 구현되기 전의 JavaScript에는 모듈 기능이 없었다. 그래서 사용하던 것이 CJS(CommonJS), AMD(Asynchronous Module Definition), UMD(Universal Module Definition).
...
`import.meta`는 모듈에서만 접근할 수 있는 메타 프로퍼티(meta-property)다. 만약 모듈이 아닌 브라우저 콘솔 같은 스크립트 영역에서 호출하면 `SyntaxError`가 발생한다:
...
`import.meta`의 프로퍼티가 어떤 것이 있어야하는지 정해진 적은 없지만 일반적으로 구현되는 속성들이 있다. 그 중 `import.meta.url`은 모듈에 대한 전체 URL(프로토콜부터 쿼리 스트링까지)을 반환한다.
