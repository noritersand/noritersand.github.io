...
Git 데이터베이스에서 누락된 객체를 검사하고, 복구 가능한 커밋은 `.git/lost-found/other` 디렉토리에 복구한다고 한다. 데이터 손실 가능성이 있기 때문에 실행 전에 Git 저장소를 백업하는 게 좋다고...

스테이징 상태에서 hard reset을 했을 때, 잃어버린 변경사항을 복구했다는 제보가 있다.
