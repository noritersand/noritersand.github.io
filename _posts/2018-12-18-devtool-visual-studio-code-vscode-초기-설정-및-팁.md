...
- Go to Next/Previous Member `mishkinf.goto-next-previous-member`: 전과 후의 멤버(함수, 메서드, 프로퍼티, 지역변수 등)로 이동하는 기능을 추가한다. Windows 버전의 기본 단축키는 <kbd>ctrl + up</kbd> <kbd>ctrl + down</kbd>
...
Show All Commands에서 `Preferences: Open Keyboard Shortcuts (JSON)` 입력하면 `keybindings.json`이 열리는데, 내용을 아래처럼 변경한다:
