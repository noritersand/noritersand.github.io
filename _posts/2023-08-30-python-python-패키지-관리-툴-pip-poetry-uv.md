...
`uv run`은 지정한 스크립트나 명령을 프로젝트의 가상환경에서 실행하는 명령이다. 만약 `FILE_NAME`을 `abc`처럼 확장자 없이 지정하면, uv는 관리 중인 가상환경의 `abc`라는 실행 파일이나 명령를 찾아 실행하려고 한다. (이 경우 실행 파일은 보통 `.venv/Scripts` 아래에 `.exe`로 존재함)
