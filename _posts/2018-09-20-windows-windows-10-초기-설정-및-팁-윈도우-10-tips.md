...
✂️ 중복 서술이 있어서 [이 블로그 내부 링크 \| Windows 노트](/windows/windows-windows-노트-윈도우-notes/)로 일부 이동함
...
~~`설정 > 장치 > 입력 > 고급 키보드 설정`에서 '이모티콘을 입력한 후 자동으로 패널을 닫지 않음' 체크.~~
...
말이 최적화 설정이지 하드웨어만 좋으면 이런거 안해도 됨.

### 백그라운드 앱 끄기

~~`설정 > 개인 정보 및 보안 > 백그라운드 앱`에서 앱 찾아서 끄기.~~
...
- <kbd>win + ctrl + 숫자키</kbd>: 작업 표시줄에 고정된 앱의 마지막 활성 창으로 전환한다. 같은 앱의 인스턴스가 둘 이상 실행되어 하나의 작업 표시줄 자리에 겹쳐있는 상황에서만 의미 있는 단축키.
