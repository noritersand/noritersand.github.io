...
0개 이상의 문자를 검색(모든 것이라는 의미 → `{0,}` 같은 의미). 항상 어떤 문자 다음에 위치해야 한다.

`por*`는 `po` 다음에 `r`이 0번 이상 존재하는 문자열을 찾는다. 0번 이므로 r이 없는 경우도 포함해 `po`, `por`, `porr` 등을 찾는다.

줄 바꿈을 제외한 모든 문자를 의미하는 `.`과 조합하여 모든 문자열 검색에 활용할 수 있다. 가령 `qwer`을 포함하는 라인 전체를 선택하려면 `.*qwer.*`라고 작성한다.
