...
| `^`      |  입력 문자열의 시작 위치를 검색. `^A` 는 검색하고자 하는 문장의 시작문자가 A인지를 검사. 쉽게 설명하면 `^qwe`는 어떤 문자열이나 라인이 정확히 'qwe'로 시작하는지를 검사하라는 의미임. |
| `$`      |  입력 문자열의 끝 위치를 검색. `A$` 는 검색하고자 하는 문장의 마지막문자가 A인지를 검사. `qwe$`는 문자열이나 라인이 정확히 'qwe'로 끝나는지를 검사하라는 의미다. 위의 `^`와 조합하면 `^qwe$`인데 문자열이나 라인에 정확히 'qwe'만 존재하는지를 검사한다. |
...
| `\D`     |  비 숫자 문자를 찾는다. `[^0-9]` 혹은 `[^\d]`와 같은 의미                                                                                               |
...
| `\W`     |  문자가 아닌 요소, `%` 같은 특수 문자를 의미함(`[^A-Za-z0-9_]`와 같은 의미)                                                          |
| `\n`     |  n은 마지막 일치하는 문장                                                                                                                |
