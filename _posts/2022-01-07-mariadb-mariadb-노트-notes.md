...
`set` 다음에 `@`로 시작하는 변수를 초기화할 수 있음. 한 번 실행하면 세션이 끊어질 때까지 유지됨.

```sql
set @orgNo = 84;

select * from visit_request where orgNo = @orgNo;
```
...
ℹ️ Oracle 호환용 함수인 `ROWNUM()`이 있다.

#### 사용자 정의 변수로 ROW NUMBER 구현하기

```sql
select @rnum := @rnum + 1 as rnum
from test_table, (select @rnum := 0) r
```

사용자 변수인데 `SET`이 없어서 이상해 보이지만, 잘 보면 서브 쿼리에서 `@rnum`을 초기화 한다.

### VALUE(), VALUES()

INSERT ON DUPLICATE KEY UPDATE 구문에서만 쓰이는 함수로, 원래 INSERT 하려고 했던 값을 반환한다. 

```sql
insert into test_value_functions(id, co1, co2)
values (3, 1033, 'cc')
on duplicate key update
    co1 = value(co1) + 1,
    co2 = concat(value(co2), 'c')
```

그래서 위 쿼리를 실행하면 키 중복이 없을 때 `{"co1": 1033, "co2": "cc"}` 데이터가 생성되고, 키 중복이 있으면 해당 로우가 `{"co1": 1034, "co2": "ccc"}`로 수정된다.

`VALUES()`는 `VALUE()`와 이름만 다르지 완전히 동일한 기능이다. 원래 `VALUES()`만 있었는데, 표준 SQL과의 충돌을 피하기 위해 `VALUE()`로 이름을 바꿨다고 한다. 
