...
위와 같은 경고창이 나타나며 연결이 안될때, 다른 원인일 수도 있지만 일단 방화벽 장비의 포트 차단이 의심된다면 원격 접속 포트를 변경해보자. 어차피 밑져야 5분 손해다. 다음은 Windows 원격 접속 기본 포트인 3389를 임의의 포트 9093으로 변경하는 방법이다.
...
를 3389에서 원하는 포트로 변경 후, Windows 방화벽 인바운드 규칙(9093 포트 허용)을 생성 후 재시작한다.
