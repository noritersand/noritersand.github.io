...
증가/증감`++ --` 연산자는 피연산자를 가능할 경우 숫자 타입으로 변경한다. `x++`에서 `x`가 문자열 `"1"`이면 `x`는 숫자 1로 바뀌며 이후 1씩 증가된다. `x`가 숫자로 바꿀 수 없는 문자열일 경우 `NaN`을 반환한다.
...
`==`는 *동등 연산자*, `===`는 *일치 연산자* 혹은 *엄격한 동등 연산자*라고 한다. `==`는 equality를 판단하며 `===`는 identical을 판단한다. 두 연산자는 같음을 정의하는 기준이 다르다. 예를 들어 `===`는 값이 같아도(타입 변환을 통해 같다고 판단되는 값. 1과 '1'은 동등하다.) 타입이 다를 경우 둘이 서로 다른것으로 판단한다. 
...
- JavaScript에서는 유니코드 문자열에 대해서 정규화 과정을 수행하지 않는다. 또한 `===`나 `==` 연산자를 사용해 유니코드 문자열의 동등 비교를 할 수 없다. 이와 같은 문자열을 비교하려면 `String.localeCompare()`를 사용한다.
...
만약 피연산자가 불리언이 아닐 때 해당 피연산자는 불리언 타입으로 변환된다. 이후 규칙에 따라 값을 반환하는데, 반환되는 값은 불리언이 아니라 불리언으로 변환되기 전인 원래의 값이 반환된다. 
