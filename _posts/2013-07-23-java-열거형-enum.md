...
- <https://bluepoet.me/2012/07/18/번역자바-enum의-10가지-예제/>


## enum

enum은 Java 1.5부터 추가된 자료형이다.
...
`Enum`이 기본으로 제공하는 `valueOf()` 메서드는 못찾으면 `IllegalArgumentException`이 발생해서 귀찮다.

```java
public enum CrudMode {
    READ, CREATE, UPDATE, DELETE, NOTHING;
...
```java
public enum CrudMode {
    READ("R"), CREATE("C"), UPDATE("U"), DELETE("D"), NOTHING("N");
     ...
     * label이 일치하는 enum 반환
     */
    public static CrudMode from(String label) {
        for (CrudMode ele : CrudMode.values()) {
...
⚠️ 용도에 따라선 추가 필드 없이 쓰는 게 나을 수도 있음. 만약 위 예시처럼 별도의 핃드가 있는 enum을 MVC 컨트롤러에서 입력값으로 받으려면 **enum 마다 스프링 컨버터를 등록해야 한다**. 😑
