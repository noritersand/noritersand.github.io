...
만약 enum을 `READ("R")` 이런 식으로 어떤 파라미터를 부여하며 선언했다면, 그 파라미터가 일치하는 클래스를 찾아 반환하는 메서드를 만들 수 있다.
...
public enum CrudMode {
     ...
     * @return
     */
    public static CrudMode of(String name) {
        CrudMode[] values = CrudMode.values();
        for (CrudMode ele : values) {
            if (name.equals(ele.value())) {
                return ele;
            }
        }
        return null;
