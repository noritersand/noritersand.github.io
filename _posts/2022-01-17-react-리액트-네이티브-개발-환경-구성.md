...
메트로를 실행중인 터미널은 그대로 둔다. 새 터미널을 열고 아래 입력:
...
리액트 네이티브에 기본 내장된 웹 버전 디버거와 앱 버전 디버거가 있고, 일단 연결되면 마치 브라우저 개발자 도구 쓰듯이 쓸 수 있다. (콘솔에 출력하기, `debugger`로 브레이크 걸기 등)
