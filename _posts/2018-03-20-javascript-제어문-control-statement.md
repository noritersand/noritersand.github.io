...
조건식의 값과 `case` 레이블의 값이 일치하면 `case` 다음에 오는 코드 블록을 실행한다. 만약 일치하는 `case` 레이블이 없으면 `default` 레이블의 코드 블록을 실행한다. `default`는 생략할 수 있으며, 위치는 `break`를 생략하지만 않으면 어디에 있어도 상관 없다.
