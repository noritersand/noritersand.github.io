...
- `/J`: 버퍼링되지 않은 I/O를 사용하여 복사한다. 큰 파일을 복사할 때 성능 향상이 있을 수도 없을 수도...
- `/MT[:N]`: 멀티스레드로 복사한다. `N`을 생략하면 기본값 8개 스레드 사용.
- `/XO`: 원본보다 오래된 대상 파일은 복사하지 않는다. 이미 최신 파일이 있는 경우 불필요한 복사를 줄일 수 있다.
- `/XC`: 변경되지 않은 파일은 복사하지 않는다.
- `/XN`: 원본보다 최신인 대상 파일은 복사하지 않는다.
