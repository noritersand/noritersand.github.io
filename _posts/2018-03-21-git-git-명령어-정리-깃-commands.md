...
pick-me 브랜치에 파일 `CONFLICT_ME.md`를 수정한 커밋이 세 개가 있으며 main 브랜치에 체리픽으로 일부만 반영하려는 상황이라고 가정하자. 이때 수정한 커밋 중 가장 마지막 커밋을 지정해 체리픽을 하면 세 번째 커밋의 변경 사항만 main 브랜치에 반영될 거라 짐작할 수도 있으나...
...
```bash
# 이 옵션은 경로를 별도로 지정하지 않으면 하위 디렉터리를 삭제하지 않음
git clean -f .

# 하위 디렉터리와 .gitignore로 무시된 파일까지 모두 삭제
git clean -dfx
```
...
- `--min-parents=<number>` `--max-parents=<number>`: 부모 커밋이 최소(혹은 최대) 몇 개가 있는 커밋인지를 특정할 때 사용하는 옵션이다. `--max-parents=1`은 부모 커밋이 하나인 커밋만 출력하라는 의미라 `--no-merges`와 같고, `--min-parents=2`는 부모 커밋이 최소 2개여야 한다는 의미니까 `--merges`와 같다.
...
- `--diff-filter=[(A|C|D|M|R|T|U|X|B)...[*]]`: 커밋 정보에 포함된 파일의 상태(추가, 수정 등)로 필터링하는 옵션. 특정 상태만 보고 싶으면 대문자로, 제외하려면 소문자로 작성한다. `--diff-filter=A`는 추가(Added)된 파일만 출력하고, `--diff-filter=a`는 추가된 파일을 제외한 나머지 전부를 출력한다.
...
- `--soft`: HEAD만 target 커밋으로 옮긴다. 인덱스와 워킹 트리는 유지한다.
- `--mixed`: 기본값. HEAD와 인덱스를 target 커밋으로 옮긴다. 워킹 트리는 유지한다.
- `--hard`: HEAD, 인덱스, 워킹 트리를 모두 target 커밋에 맞춘다. **로컬 변경이 삭제될 수 있다**.
- `--keep`: HEAD와 인덱스를 target으로 옮기고 워킹 트리는 유지한다. 단, HEAD와 target 사이에서 내용이 바뀌는 파일을 현재도 수정 중이면, 그 변경사항을 덮어쓸 수 있으므로 reset을 중단한다.
- `--merge`: HEAD와 인덱스를 target으로 옮기고 워킹 트리는 유지한다. 단, target과 index 사이에서 변경되는 파일에 unstaged 변경이 있으면 reset을 중단한다.

`--keep`과 `--merge`는 `--mixed`와 비슷하게 작동하지만, 로컬 변경사항과 충돌할 가능성이 있으면 reset을 중단하는 것이 다르다. `--keep`은 최근 커밋을 없애면서 현재 작업을 유지할 때 쓰이고, `--merge`는 충돌난 merge 작업을 되돌릴 때 쓰인다.
...
특정 파일이나 파일 구조(트리)에 대한 해시를 계산하려면 아래처럼 입력한다:
...
`git worktree`는 하나의 Git 저장소에서 여러 개의 워크트리(worktree)를 동시에 다룰 수 있게 해주는 명령어다.
...
원래 브랜치를 전환하려면 변경사항을 커밋하거나 스태시에 임시 저장해야 한다. 여기서 `git worktree`를 사용하면 기존 워크트리를 그대로 두고 새로운 디렉터리에 별도의 워크트리를 만들어 다른 브랜치로 작업할 수 있다. 이 방식은 예를 들어 긴급한 수정이 필요할 때, 기존 작업을 건드리지 않고 새 브랜치에서 바로 진행할 수 있다는 점이 매우 유용하다.

#### ⚠️ 워킹 트리(working tree)와 워크트리(worktree)

이름이 또 하필 워크트리라서 기존의 워킹 트리와 헷갈린다. 실제로 의미가 거의 비슷하긴 한데, 엄밀히 따지면:

- 워킹 트리는 버전 관리되는 파일이 실제로 존재하는 작업 공간 자체를 가리키는 일반 개념이다.
- 워크트리는 문맥에 따라 `git worktree` 명령 또는 해당 명령으로 만들어지는 개별 작업 공간을 가리키는 말이다. 후자의 경우 인스턴스의 개념으로 이해하면 된다.
- 원래부터 있던 인스턴스는 메인 워크트리(main worktree), 메인 워크트리를 바탕으로 새로 만들어진 인스턴스는 연결된 워크트리(linked worktree)라고 부른다.
...
# ⚠️ 메인 워크트리가 main 브랜치를 보고 있으면 실행 안됨
git worktree add ../another-repo main
```

위 명령들은 `../another-repo` 디렉터리에 특정 브랜치를 위한 새로운 워크트리를 생성한다. `-b test`는 `test` 브랜치를 새로 만들면서 체크아웃 하겠다는 의미다.

새 워크트리는 가능한 한 기존 워크트리의 하위 디렉터리가 아닌, 별도의 경로로 지정하는 것이 좋다. 기존 경로 하위에 만들면 Git이 마치 새 파일들이 잔뜩 생긴 것처럼 인식하기 때문.

#### 워크트리 확인/삭제

생성된 워크트리는 겉보기에 독립된 Git 저장소처럼 보이지만, 실제로는 기존 워크트리와 같은 Git 저장소를 공유한다. (디렉터리 내부를 살펴보면 `.git/` 대신 기존 저장소를 가리키는 `.git` 파일이 존재하는 것을 확인할 수 있음) 그래서 다른 워크트리가 이미 체크아웃한 브랜치로는 전환할 수 없다.
...
생성한 워크트리 목록 확인은 `git worktree list`로, 삭제는 `git worktree remove`로 하면 된다. 워크트리를 삭제할 때 실제 디렉터리도 전부 삭제되니 변경사항을 날려먹지 않도록 주의하자.

`git worktree prune`은 이미 디렉터리는 삭제되었지만 `.git/worktrees/` 아래에 남아 있는 메타데이터만 정리한다. 예를 들어 워크트리를 수동으로 삭제한 경우, Git은 해당 워크트리가 여전히 존재하는 것으로 간주하므로 `prune`으로 정리해야 한다.

ℹ️ 작업 완료 후의 머지는 Git 저장소를 공유하니까 원래의 워크트리에서 평범하게 하면 됨.
