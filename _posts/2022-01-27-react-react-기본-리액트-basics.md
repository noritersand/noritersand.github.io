...
`eject`를 한 번 실행된 후에 다시 `CRA`로 되돌릴 방법은 버전 관리 시스템에서 롤백하는 것 외엔 없다.
...
1. `setName('Waldo')`는 버튼 클릭 이벤트 핸들러가 실행되고 있는 현재 이벤트 루프에서 즉시 호출됨 -> 첫 번째 렌더링
2. 1초 뒤 `setTimeout()` 콜백(= 다른 이벤트 루프) 안에서 3번의 `setCount()`가 발생. 이 3번의 `setCount()`는 같은 타이머 콜백(동일 이벤트 루프) 내에서 연속 호출되므로, 한 번에 배치되어 렌더링을 딱 한 번만 더 일으킴 -> 두 번째 렌더링 발생
