...
Vue를 JavaScript 라이브러리처럼 사용하는 방법이다. 이쪽은 스크립트 태그 혹은 CDN 방식이라 한다.
...
현재(🗓️ 2023-03-06) 빌트인 디렉티브는 이마안큼 있다.
...
콧수염(Mustache syntax)이나 Vue 디렉티브 속성의 내부에 작성한 코드는 JavaScript 표현식으로 작동한다.
...
모든 JavaScript 코드가 허용되는 것은 아니다. 도움말에 따르면 하나의 단일 표현식(one single expression)만 가능하며, 값으로 평가되는 표현식이어야 한다고 한다. 메서드 호출은 가능하지만, 선언식이나 반환식은 허용되지 않는다. 그리고 블록`{}`도 사용할 수 없으며 구문(statements)도 불가능.

#### 전역 변수의 접근 제한

JavaScript 표현식에서 접근 가능한 전역 참조는 정해진 것 외에는 불가능하다. 예를 들어 사용자 정의 전역 변수와 window는 표현식에서 쓸 수 없다. 대신 메서드를 통한 간접 참조는 가능함(표현식에선 메서드를 호출하고. 메서드에서 전역 변수에 접근).
