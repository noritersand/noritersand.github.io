...
예를 들어 `&#58;`를 출력할 때 `escapeXml="false"`일 땐 `&#58;` 문자 그대로 내보내고, 브라우저가 이를 `:`으로 치환한다. 반면 `escapeXml="true"`면 `&#58;`를 그대로 출력하기 전에 한 번 더 escape 해서 `&amp;#58;`라는 문자를 응답한다. 브라우저가 보기에 `&amp;#58;`에서 치환해야 하는 문자는 `&amp;` 뿐이므로 최종적으로는 `&;#58;`가 페이지에 표시되는 것이다.
