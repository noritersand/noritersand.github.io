...
GCM(Git Credential Manager)의 하위 개념. 

`credential.helper`가 GCM일 때만 유효하며, 요청 URL을 보고 GitHub/GitLab/Bitbucket 등 어느 서비스인지 자동 판별해 적절한 인증 흐름(브라우저 팝업, 기기 코드 등)을 선택하는 내부 컴포넌트다. 필요 시 `GCM_PROVIDER` 환경변수 등으로 특정 provider를 강제 지정할 수도 있다.

자동 판별이 실패하거나 강제로 지정하고 싶으면 `credential.provider` 설정으로 직접 명시할 수 있다. 생략했을 때의 기본값은 `auto`.
...
credential helper(manager)가 인증 정보를 실제로 어디에 저장할지 지정하는 설정이다. `gpg`, `wincredman`, `plaintext` 등의 값으로 설정할 수 있고, 생략했을 때의 기본값은 환경에 따라 다르다. (Windows는 `wincredman`)
...
⚠️ 이렇게 설명하면 Host의 자격 증명을 같이 사용함. Host와 WSL의 인증을 분리하고 싶다면 WSL 파일 시스템에 위치한 파일을 지정할 것


## GPG 키 설정하기

ℹ️ GPG 설정과 온갖 디버깅이 번거롭다면, `credential.credentialStore=secretservice`도 있긴 하다. 인증정보를 평문으로 저장하는 것보다 약간 나은 수준이라 별로 권장은 안하지만...
