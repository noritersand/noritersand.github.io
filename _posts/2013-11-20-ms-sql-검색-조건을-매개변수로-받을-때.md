...
만약 `NVL()`을 사용한다면 `'' = ''` 대신 `1 = 1`(결과가 true인 비교식이면 상관없음. 단 오라클의 경우, 데이터 타입에 주의할 것. 가령 매개변수가 숫자일때 `'a' = 'a'`는 에러를 발생한다. 

특이하게도 `'1' = '1'`은 모든 숫자와 문자 타입 모두 대응한다) 로 작성한다. 그리고 `NVL()` 함수는 빈문자열도 `NULL`로 취급한다(MS-SQL에선 `'' != null` 이며 `'' = ''`는 true이지만 oracle은 `'' == null` 이며 `'' = ''` 는 false다).
