...
master 브랜치에 feature 브랜치를 머지하는 상황이라 가정했을 때, 머지한 내용을 되돌리고 싶으면 master 브랜치를 revert 하는게 아니라 feature 브랜치를 revert하고 그 다음 다시 머지하는게 낫다.  
master를 revert 한 뒤에 feature에서 같은 라인을 수정 후 다시 머지하려고 하면 충돌이 발생하거나 변경사항이 없다며 머지가 아예 안될 수도 있기 때문.  
만약 feature 브랜치의 커밋이 잘게 쪼개져있어서 모두 revert하기 어려운 상황이라면 master를 revert하되 revert 커밋을 feature로 역머지 후 작업하는 방법이 있긴 하다.
