...
- 데이터베이스
  ...
  - 접두어: sql
- 함수(Functions)
  - 기능: 컬렉션 프레임워크 타입 처리, String 처리
  - 접두어: fn

## 환경설정

기존의 컨텍스트에서 JSTL을 사용하려면 웹 애플리케이션의 WEB-INF/lib 디렉터리에 필요한 라이브러리를 위치시킨다. JSTL의 주된 라이브러리 파일은 jstl.jar, standard.jar이고, XML에서 지원되는 기능을 사용하려면 jaxen-full.jar, saxpath.jar, jaxp-api.jar 파일 등이 필요하다.

<http://tomcat.apache.org/taglibs/standard/>

혹은 `톰캣설치경로/webapps/examples/WEB-INF/lib` 에서 가져온다.
