...
- 허브(hub): 그물형(mesh)\*으로 구성된 네트워크의 비효율성을 개선하기 위한 신호 중계 장치. 하지만 받은 데이터를 연결된 모두에게 전송하는 한계가 있음
- 스위치(switch): 허브를 보완하여 특정 컴퓨터에게만 데이터를 전송하게 하는 데이터 방향 전환 장치. 요즘 나오는 허브들은 스위칭 기능을 지원하기도 함.
- 라우터(router): 여러 스위치로 구성된 LAN의 대표 역할을 하는 장치. 라우터를 통해 다른 LAN과 연결한다. WAN을 구성하는 LAN의 관문 역할이라 할 수 있다.
- 게이트웨이(gateway): 스위치와 라우터의 중간에 위치하여 데이터 전송을 위해 패킷의 프로토콜을 변환하는 장치

\* 그물형: 네트워크에 접속하는 모든 장치가 다른 모든 장치에 연결되는 형태.
