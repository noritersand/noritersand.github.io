...
객체 자신이 소유한 프로퍼티가 아니면 반환되지 않는다. 예를 들어 아래처럼 새 객체 `{a: 1}`는 소유한 프로퍼티가 `a`만 있으므로 `toString`의 프로퍼티 설명자는 안나온다:
...
`proto`가 `null`이면 JavaScript의 순수 사전식(pure dictionary) 객체인 null 프로토타입 객체(null-prototype objects)가 만들어진다.
