...
SaaS(Software as a Service)는 노션, 드랍박스 처럼 인프라에서 애플리케이션까지 기업이 모든 것을 관리하는 서비스를 말한다.

IaaS(Infrastructure as a Service)는 OS와 인프라 영역(가상화, 서버, 저장장치, 네트워)을 기업에서 관리하는 서비스다. 대표적으로 AWS EC2가 있음.

PaaS(Platform as a Service)는 SaaS와 IaaS의 중간 단계로 인프라와 런타임 환경, 미들웨어, OS까지 기업에서 관리하는 서비스다. 구동할 애플리케이션의 소스 코드만 사용자가 업로드하는 방식이라고 보면 됨.
...
언젠가부터(아마 약 10년 전 부터) 사용되기 시작한 메서드 이름. 보통은 인스턴스를 새로 만들어주는 메서드일 때 사용된다. 전달인자가 있는 경우 새 인스턴스를 만들 때의 변수로 활용된다.
...
상용 APM 툴로 Pinpoint, Scouter, 제니퍼, 와탭이 있다. (예시가 다 국산인 건 우연의 일치)
