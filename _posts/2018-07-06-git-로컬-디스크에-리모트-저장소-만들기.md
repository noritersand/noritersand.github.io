...
일반적인 Git 저장소(리모트 저장소가 따로 존재하는 로컬 저장소)는 'git init'으로 생성하지만 원격(서버) 저장소라면 bare-repository로 생성해야 한다. bare-repository는 워킹 트리가 없고 변경 사항만 추적하는 저장소를 말한다.

bare-repository를 생성해서 리모트 저장소로 등록해보자.
...
file 프로토콜 사용여부에 따른 차이는 [여기](https://git-scm.com/book/ko/Git-서버-프로토콜)에서 확인할 수 있다.
...
그리고 다음처럼 Git 저장소 등록 후 서버를 추가한다:
