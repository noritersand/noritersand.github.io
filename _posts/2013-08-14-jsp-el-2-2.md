...
EL은 JSP 페이지에 사용되는 Java 코드를 대신해 값을 표현하는데 사용된다.
...
- EL은 달러 기호(`$`) 와 표현식 그리고 중괄호(`{ }`)를 사용해서 표현한다.
- EL은 숫자, 문자열, boolean 값과 null 같은 상수 값(리터럴)들도 포함할 수 있다.
- 스크립릿 내부에서 쓸 수 없다. 즉, `<% %>`, `<%! %>`, `<%= %>` 내에서 사용할 수 없다.
- 표현식 안에 연산식도 작성이 가능 하다.
- 프로퍼티 접근 연산자(혹은 속성 접근자) 마침표(`.`)는 EL에게 Java Bean이나 맵/컬렉션 객체에서 다음에 오는 이름과 같은 프로퍼티를 찾게 한다. ex: `${article.num}`
- 프로퍼티 접근 연산자 대괄호(`[ ]`)에 지정된 값이 문자열일 땐 마침표(`.`)와 동일하게 작동한다. 지정된 값이 숫자일 땐 배열로 인식하고 해당 숫자와 동일한 인덱스를 찾는다. ex: `${article['num']}`, `${arry[0]}`
...
위처럼 각 객체에 할당한 속성에 접근할 때 아래와 같이 사용한다:
