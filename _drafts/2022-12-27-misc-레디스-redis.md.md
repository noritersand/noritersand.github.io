...
RDB는 저장 옵션(persistence option)의 기본값이다. RDB는 주기적으로 스냅샷을 파일로 저장하는 방식이다. (백업이 기본으로 되는건지는 확인 필요)
...
레디스 서버 2대 이상을 클러스터링으로 구성하면 RDB 옵션이어도 디스크 저장 없이 재난 대처가 가능한다. 죽었다 살아난 서버가 다른 클러스터의 데이터를 불러오도록 할 수 있기 때문.
