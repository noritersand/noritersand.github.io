...
- 웹 워커는 스크립트를 백그라운드 스레드에서 실행하게 해줌. 메인 실행 쓰레드와 분리되어 실행되기 때문에 메인 쓰레드를 차단하거나 느리게 만드는 것을 방지할 수 있음.
...
```
- [이건 서비스 워커](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API)인데, 백그라운드 실행이라는 개념은 비슷하나 훅을 사용할 수 있다는데 자세히 안봄
> 요약하면, Web Workers는 메인 스레드와 별개의 스레드에서 병렬 작업을 수행하는 기술이고, Service Workers는 백그라운드에서 네트워크 요청을 제어하고 캐싱하여 오프라인 및 빠른 로딩을 지원하는 기술입니다. 두 기술은 각각의 목적과 동작 방식으로 웹 애플리케이션의 성능과 기능을 향상시키는 데에 기여합니다. - openai.com

### [JavaScript] Streams API
- [https://developer.mozilla.org/en-US/docs/Web/API/Streams_API](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API)
...
> 다들 욕만 하고 그 와중에 돈이 안되니까 절약하고 살려고 로씨아로 돌아갔는데
...
- 때문에 프론트 소스(js)에 이 토큰을 포함시키면 안 되는데
...
- `_drafts\2023-04-05-web-웹소켓-websocket.md`
...
- [어떤 이의 말](https://stackoverflow.com/questions/27538706/when-to-throw-runtime-exception)에 따르면 `RuntimeException`은 '프로그램이 실행되는 환경을 변경하는 대신 코드를 변경해야만 해결할 수 있는 프로그래밍 문제'를 나타내며, 유효하지 않은 매개변수 전달, 잘못된 순서의 메서드에 사용한다고... 여기서 '유효하지 않음'이란 비즈니스적인 의미가 아니라 `null` 같은 프로그래밍적으로 유효하지 않음을 말한다.
