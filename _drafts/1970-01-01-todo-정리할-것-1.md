...
### [Java] program arguments 와 VM arguments의 차이
- program arguments: main 클래스의 전달 인자
```bash
java MainTest a 1 b c 3 4 d f
```
- VM arguments(=시스템 프로퍼티<sup>system properties</sup>): java 실행 할 때 `java` 명령어의 `-D` 옵션으로 지정하는 값
```bash
java MyApp -Dspring.profiles.active=real
```

### [Java] VM arguments(system property)는 어디에서 읽을 수 있나?
  - java
  - jvm
  - system-properties
  - vm-arguments
