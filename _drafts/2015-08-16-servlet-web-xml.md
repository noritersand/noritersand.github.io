...
- `description`: 웹 애플리케이션에 대한 설명을 나타낸다.
- `distributable`: distributable 요소가 있다는 것은 웹 애플리케이션이다 중 서버 간에 분산 배치 될 수 있다는 것을 의미한다.
- `context-param`: 애플리케이션의 초기화 파라미터를 선언하는데 사용
...
- `mime-mapping`: 특정한 mime형을 가진 파일을 웹 애플리케이션에  넣어 두고 싶은  경우 사용
...
- `env-entry`: 웹 애플리케이션의 환경항목을 선언한다.
