...
변수가 객체를 어떤식으로 가리키냐의 차이. 데이터가 참조 타입일 때만 달라짐. 우선 call-by든 pass-by든 결국 같은 개념이고, 누군가의 정의로 생긴 개념이 아닌지라 말하는 사람마다 어떤게 by-reference인지, by-value인지 주장하는 바가 조금씩 다르다.

많은 사람들이 자바나 자바스크립트, 파이썬 등의 언어를 pass-by-value라고 말하곤 하는데 엄밀히 따지면 [pass-by-object-reference](https://robertheaton.com/2014/02/09/pythons-pass-by-object-reference-as-explained-by-philip-k-dick/)라는 말이 있다. 이에 따르면 분류는 둘이 아니라 셋이다.

pass-by-reference는 메모리 주소를 변수에 저장한다. 함수에 전달할 땐 변수의 값이 아니라 변수 자체를 가리키는 참조를 전달하므로, 함수 안밖의 변수는 서로 다르지 않고 완전히 일치하며 늘 같은 객체를 가리킨다. 이것은 재할당 후에도 마찬가지다. C언어 등의 포인터를 상상하면 된다.

pass-by-value는 변수에 객체를 값으로 할당한다. 변수의 재할당은 변수에 새로운 객체'값'을 할당함을 의미한다. 함수에는 변수의 값만을 인자로 전달하며, 전달한 값은 함수의 매개변수에 복사한다. 이 때 함수 밖 변수와의 연결은 완전히 분리된다고 봐야한다. 대부분의 언어에서 원시형 데이터를 다루는 방법이기도 하다.

pass-by-object-reference는 메모리 주소를 값으로만 취급한다(Object references are passed by value). 변수의 재할당은 변수에 새로운 메모리 주소'값'을 할당함을 의미한다. 함수에 인자로 전달한 값을 함수의 매개변수에 복사하여 사용한다는 점은 pass-by-value와 같으나 주소값을 복사했으므로 함수 안밖의 변수들은 결국 같은 객체를 가리킨다. 만약 함수에서 변수를 재할당하면, 그 시점부터 두 변수는 서로 다른 객체를 가리키게 된다.
