...
cp -a ... ...  # 소유자 등의 각종 파일 정보를 유지하며 복사
```

주의: 파일 복사나 이동은 대상 경로의 존재여부에 따라 결과가 달라진다. 복사/이동하려는 대상이 디렉터리일 때, 목적지가 존재하지 않으면(부모 디렉터리까지 없으면 에러) 지정한 경로 그대로, 목적지가 존재하면 그 아래로 복사/이동된다. [무슨 말인지 모르겠으면 이 문서를 보자](https://zetawiki.com/wiki/리눅스에서_폴더_통째로_복사하기).
